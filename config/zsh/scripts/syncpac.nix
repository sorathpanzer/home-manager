...
home.file = {
  ".config/zsh/scripts/syncpac".text = ''
#!/bin/sh
...
if [ "$(ls -A /run/media/sorath/Legion)" ]; then
  ...
  $SUDOAS syncoid --no-privilege-elevation root@192.168.1.120:Bunker/Crypta Legion/Crypta
  $SUDOAS syncoid --no-privilege-elevation root@192.168.1.120:Bunker/Vault Legion/Vault
fi
}
'';
};
